...
`;
      ...
      // 🆕 写真家モードの場合のタグ抽出特化処理（カスタムプロンプト使用時は適用しない）
      if (!customPromptInfo.hasCustom && 
...
【抽出カテゴリ（9カテゴリ・全ファイル形式対応）】
🎯 **被写体・内容タグ**: 人物,動物,キャラクター,文書内容,オブジェクト
🎨 **色彩・スタイルタグ**: 主要色,色調,デザインスタイル,パレット,トーン
📐 **構図・レイアウトタグ**: アングル,フレーミング,文書構造,バランス,配置
💡 **光線・品質タグ**: 自然光,人工光,画質,スキャン品質,明度,コントラスト
🎭 **雰囲気・トーンタグ**: 感情,印象,ムード,文書の性質,スタイル
📍 **場所・用途タグ**: 環境,背景,ロケーション,文書用途,設定
🎪 **ジャンル・分野タグ**: 撮影目的,アート分野,文書種別,カテゴリ
⚙️ **技法・形式タグ**: カメラ設定,画法,表現技法,文書形式,制作手法
🎨 **作品特化タグ**: デジタル,手描き,漫画,アニメ,リアル系,水彩,油彩,ベクター
...
【例】portrait, ポートレート, female, 女性, natural_light, 自然光, outdoor, 屋外, warm_tone, 暖色系, professional, プロフェッショナル, high_quality, 高品質, JPEG, digital_photography, デジタル写真

上記9カテゴリの観点から包括的にタグを抽出し、検索で発見しやすい多角的なタグセットを生成してください。
`;
        console.log('✅ 写真家モード特化プロンプト生成完了');
        console.log(`📸 プロンプト種別: ${promptSource} + タグ抽出特化`);
